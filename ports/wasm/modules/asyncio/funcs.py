...
async def wait_for(aw, timeout, sleep=core.sleep):
    """Wait for the awaitable to complete, but cancel if it takes longer
    ...
    try:
        ...
        # More general fetching of exception arg
...
async def gather(*aws, return_exceptions=False):
    ...
    """
    # No awaitables, nothing to gather
    if not aws:
        return []
    ...
    if state > 0:
        ...
        try:
            # Don't reschedule - done() callback will reschedule us
            await core._never()
        except core.CancelledError as er:
            cancel_all = True
            state = er
