# CircuitPython Node.js Port (wasm-node)

The CircuitPython Node.js port is a full-featured JavaScript-backed WebAssembly implementation designed specifically for Node.js environments. This port includes complete JavaScript FFI support, hardware simulation capabilities, and the full Python standard library. It's ideal for server-side Python execution, complex JavaScript-Python integration, and educational hardware simulation.

**Note**: For browser deployments, use the lightweight `ports/wasm` port instead.
